...
import { assertCentroActive, getCentroForEntity, } from './centro-custo.service.js';
import { cellToOutput, formatDateOnly, getMonthDateRange, parseDateOnly, validateCellValue, } from './cell-value.validator.js';
function mapRowToDto(registro, tipoByProp) {
    const values = {};
    for (const valor of registro.valores) {
        ...
        if (output)
            values[valor.propriedadeId] = output;
    }
    return {
        id: registro.id,
...
async function validateItemIds(prisma, entityId, centroId, itemIds) {
    if (itemIds.length === 0)
        return;
...
async function persistCellValues(prisma, entityId, centroId, registroId, values, activeConfigs) {
    if (!values)
        return;
    for (const config of activeConfigs) {
        ...
        const isEmpty = !parsed.valorTexto &&
            !parsed.valorData &&
            !parsed.valorDecimal &&
...
export async function listRegistrosDiarios(prisma, entityId, centroId, year, month) {
    ...
    const tipoByProp = new Map(centro.propriedadeConfigs.map((c) => [c.propriedadeId, c.propriedade.tipo]));
...
export async function createRegistroDiario(prisma, actorId, entityId, centroId, input) {
    ...
    await persistCellValues(prisma, entityId, centroId, registro.id, input.values, activeConfigs);
    ...
    const result = await listRegistrosDiarios(prisma, entityId, centroId, data.getUTCFullYear(), data.getUTCMonth() + 1);
...
export async function updateRegistroDiario(prisma, actorId, entityId, centroId, registroId, input) {
    await assertCentroActive(prisma, entityId, centroId);
    const centro = await getCentroForEntity(prisma, entityId, centroId);
    const activeConfigs = centro.propriedadeConfigs.filter((c) => c.active);
    ...
    await persistCellValues(prisma, entityId, centroId, registroId, input.values, activeConfigs);
    await writeTenantAudit(prisma, {
        entityId,
        userId: actorId,
...
export async function deleteRegistroDiario(prisma, actorId, entityId, centroId, registroId) {
    await assertCentroActive(prisma, entityId, centroId);
    const existing = await prisma.registroDiario.findFirst({
        where: { id: registroId, centroCustoId: centroId, entityId },
    });
