...
export function validateCellValue(tipo, input) {
    const empty = {
        valorTexto: null,
        valorData: null,
        valorDecimal: null,
...
export function cellToOutput(tipo, row) {
    if (!row)
        return null;
    switch (tipo) {
        case 'TEXTO':
...
export function isMarkerSatisfied(tipo, row) {
    if (!row)
        return false;
    if (tipo === 'BOOLEAN')
        return row.valorBoolean === true;
    if (tipo === 'DATA')
        return row.valorData != null;
    if (tipo === 'TEXTO')
        return (row.valorTexto?.trim().length ?? 0) > 0;
    return false;
}
...
//# sourceMappingURL=cell-value.validator.js.map