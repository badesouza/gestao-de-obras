import { formatDateOnly, getMonthDateRange, isMarkerSatisfied, } from './cell-value.validator.js';
import { getCentroForEntity } from './centro-custo.service.js';
/** Aggregates daily production stats for a centro in a month */
export async function getProducaoDiaria(prisma, entityId, centroId, year, month) {
    ...
    for (let day = 1; day <= daysInMonth; day += 1) {
        ...
        for (const registro of rows) {
            if (inicioConfig) {
                const valor = registro.valores.find((v) => v.propriedadeId === inicioConfig.propriedadeId);
                if (isMarkerSatisfied(inicioConfig.propriedade.tipo, valor ?? null)) {
                    iniciadas += 1;
                }
            }
            if (conclusaoConfig) {
                const valor = registro.valores.find((v) => v.propriedadeId === conclusaoConfig.propriedadeId);
                if (isMarkerSatisfied(conclusaoConfig.propriedade.tipo, valor ?? null)) {
                    concluidas += 1;
                }
            }
        }
