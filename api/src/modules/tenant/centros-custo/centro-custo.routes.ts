...
export async function registerCentroCustoRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        /* ── agregar por dia ── */
        const porDia: Record<string, { data: string; total: number; concluidos: number; area: number }> = {};
        ...
        const porEquipe:  Record<string, { registros: number; area: number; horas: number }> = {};
        ...
        registros.forEach(reg => {
          ...
          reg.valores.forEach(v => {
            if (v.valorTexto  != null) valMap[v.propriedade.nome] = v.valorTexto;
            if (v.valorDecimal != null) valMap[v.propriedade.nome] = Number(v.valorDecimal);
          });
          ...
          if (origem) porOrigem[origem] = (porOrigem[origem] ?? 0) + 1;
          if (equipe) {
        ...
        const toArr = (obj: Record<string, number>, limit = 10) =>
          Object.entries(obj)
  ...
  fastify.get(
    '/centros-custo/:id/metas/ano',
    { preHandler: canView },
    async (request, reply) => {
      try {
        const { id } = request.params as { id: string };
        const entityId = request.user.entityId!;
        const year = Number((request.query as Record<string, string>).year ?? new Date().getFullYear());

        const rows = await fastify.prisma.$queryRaw<Array<{
          month: number;
          meta_registros: string | null;
          meta_producao:  string | null;
          meta_horas:     string | null;
        }>>`
        ...
        const byMonth: Record<number, { metaRegistros: number | null; metaProducao: number | null; metaHoras: number | null }> = {};
        rows.forEach(r => {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        const meta = await fastify.prisma.$queryRaw<Array<{
          meta_registros: string | null;
          meta_producao:  string | null;
          meta_horas:     string | null;
        }>>`
