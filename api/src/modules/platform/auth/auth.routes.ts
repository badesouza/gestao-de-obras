...
import {
  buildPlatformTokenPayload,
  getPlatformOperator,
  loginPlatformOperator,
  logoutPlatformOperator,
...
export async function registerPlatformAuthRoutes(fastify: FastifyInstance) {
  fastify.post('/auth/login', async (request, reply) => {
    try {
      ...
      const token = await reply.jwtSign(
        buildPlatformTokenPayload(result.operatorId, result.email),
